...
### TV-151 — Scientific-prose constructs (SaT, default model)

With the default segmenter (`sat-12l-sm`, `weighting="hat"`) these
construct types segment correctly with no probability post-processing:

| `document` | Expected |
|-----------|----------|
| `"The results can be seen in Tab. TABREF21 and TABREF22 ."` | one sentence (no split on the `Tab.` period) |
| `"We optimize the loss in Eq. EQREF9 ."` | one sentence |
| `"...on SentiHood and SemEval-2014 Task 4 datasets. The next sentence is here."` | two sentences, split only at the period (not at the year) |
| `"We finished the work in 2014. Later, we extended the analysis."` | two sentences (a year genuinely ending a sentence still splits) |

A lighter checkpoint (e.g. `sat-3l-sm`) mis-splits these; this is a
model-quality property, not a spec violation.
