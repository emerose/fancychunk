...
The library's protocol asks the embedder to return per-sentence token
counts that sum to the row count of the embedding matrix. *How* the
embedder produces those counts is its choice. Two methods cover ~99% of
real embedders:

### Offset-based (preferred when available)

Join sentences with the empty string, tokenize once, use the
tokenizer's `offset_mapping` to map each token back to its source
sentence by character offset. **No sentinel character needed, no risk
of subword-merging surprises.** Works with any HuggingFace fast
...
stable single token across positions; join sentences with it before
...
tokens. These belong to no input sentence. Both reference adapters
absorb leading specials into sentence 0's count and trailing specials
into the last sentence's count — the conforming "option (b)" from
the spec. Choosing option (a) (an `embed_segment` that excludes
specials from its output) is also conforming if your embedder
supports it.
...
class MyEmbedder:
    ...
    def count_tokens(self, sentences: list[str]) -> list[int]:
        # Approximate per-sentence count, used by fancychunk for
        ...
        return [len(your_tokenizer.encode(s)) for s in sentences]

    def embed_segment(self, sentences: list[str]) -> tuple[NDArray, list[int]]:
        # 1. Tokenize the joined input however you like.
        # 2. Run the model, get last_hidden_state (per-token output).
        # 3. Compute per-sentence counts that conserve the row count.
